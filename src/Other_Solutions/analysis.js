...
async function analysis() {
  ...
  let maxTripDriver = await getDriver(mostTripsDriver),
    higherstEarningDriver = await getDriver(highestEarner);
  ...
  let objectReport = {
    ...
    cashBilledTotal: trips
      .filter((trip) => trip.isCash)
      .reduce((sum, trip) => {
        sum += +String(trip.billedAmount).replace(',', '');
        return +sum.toFixed(2);
      }, 0),
    nonCashBilledTotal: trips
      .filter((trip) => !trip.isCash)
      .reduce((sum, trip) => {
        sum += +String(trip.billedAmount).replace(',', '');
        return +sum.toFixed(2);
      }, 0),
    noOfDriversWithMoreThanOneVehicle: driversInfo
      .filter((driver) => driver.status === 'fulfilled')
      .filter((driver) => driver.value.vehicleID.length > 1).length,
    mostTripsByDriver: {
